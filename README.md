...
With Hugo installed run the quick install script located in the theme's `bin/install` directory, or connect to the Internet and run the following to install After Dark remotely from the [NPM package](https://www.npmjs.com/packages/after-dark) registry:

```shell
wget -qO - https://git.habd.as/comfusion/after-dark/raw/branch/master/bin/install | sh
```

Installation should complete in a matter of seconds. If you have `elinks` installed your new site [will open](https://git.habd.as/comfusion/after-dark/images/docs/feat-quick-installer.gif) automatically in a terminal browser with Hugo running in the background and available for immediate use.
